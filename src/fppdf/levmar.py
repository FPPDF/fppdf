...
def corrmatcalc(hessi, afin):
    ...
    corrmat = hessi.copy() / 2.0
    corrmati = hessi.copy() / 2.0
...
def levmar_meth2(afree):
    ...
    for nt in range(1, ntries):
        ...
        for ns in range(1, nsteps + 1):
            ...
            delchi = chi2i - chi2o
            alpha = jac @ delpar / (delchi / 2.0 + 2.0 * jac @ delpar)
            ...
            lam = lam + np.abs(delchi) / 2.0 / alpha
...
def levmar_meth3(afree):
    ...
    for nt in range(1, ntries):
        ...
        nu = 2.0
        if nt > 1:
            lammax = 1.0 - np.power(2.0 * rho - 1.0, 3)
            lam0 = np.max([lammax, 1.0 / 3.0])
            print('lammax=', lammax, lam0)
            lam = lam * lam0
    ...
    while nt < ntries:
        ...
        for ns in range(1, nsteps + 1):
            ...
            if min_pars.sgd:
                ...
                if lev_comb:
                    hess0 = hessi.copy() / 2.0 + np.diag(np.ones((pdf_pars.npar_free))) * lam
